  ...
  return (
    <div className="">
      <button
        ...
        className={`flex w-full justify-between border border-white py-4 pl-4 text-xl max-sm:text-base  ${accordionOpen && "!border-b-0 bg-white text-black"}`}
      >
        <div className=" flex-1 text-left max-sm:flex max-sm:flex-col">
          ...
          ></span>
          <span
            className={`pl-3 text-gray-400 max-sm:p-1 ${accordionOpen && "!text-gray-600"}`}
          >
            {duration}
          </span>
        </div>
      ...
      >
        <div className="overflow-hidden text-base">
          <ul
            className={`list-disc  border  border-t-0 border-white  bg-[#131417] py-4 pl-12 font-sans text-xl font-light tracking-wide text-white max-sm:pl-8 max-sm:text-base`}
          >
            {answer.map((data, idx) => (
